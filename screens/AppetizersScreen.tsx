...
interface AppetizersScreenProps {
  menuItems: MenuItem[];
  onBack: () => void;
}

const AppetizersScreen: React.FC<AppetizersScreenProps> = ({ menuItems, onBack }) => {
  const appetizers = menuItems.filter((item: MenuItem) => item.course === 'appetizers');
  const courseInfo = COURSES.find((c) => c.id === 'appetizers');
