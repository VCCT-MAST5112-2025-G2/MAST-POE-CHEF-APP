...
interface DessertsScreenProps {
  menuItems: MenuItem[];
  onBack: () => void;
}

const DessertsScreen: React.FC<DessertsScreenProps> = ({ menuItems, onBack }) => {
  const desserts = menuItems.filter((item: MenuItem) => item.course === 'desserts');
  const courseInfo = COURSES.find((c) => c.id === 'desserts');
